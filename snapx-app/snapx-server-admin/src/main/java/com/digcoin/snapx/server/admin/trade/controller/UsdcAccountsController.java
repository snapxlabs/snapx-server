...
import com.digcoin.snapx.server.admin.trade.service.UsdcAccountsAppService;
...
public class UsdcAccountsController {
    ...
    public PageResult<BaseAccountsDTO> pageAccounts(MemberQuery query) {
        return usdcAccountsAppService.pageAccounts(query);
    }

    @GetMapping("page-accounts-details")
    @Operation(description = "分页查询用户账户明细")
    public PageResult<UsdcAccountsDetailsDTO> pageAccountsDetails(@Schema(description = "会员id") @RequestParam Long memberId,
                                                                  BaseAccountsDetailsQuery query) {
        return usdcAccountsAppService.pageAccountsDetails(memberId, query);
    }
    ...
    public void updateAccounts(@RequestBody BaseAccountsModifyDTO payload) {
        usdcAccountsAppService.updateAccounts(payload);
    }
}
